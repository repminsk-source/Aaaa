...
async def handle_incoming_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Ловит ВСЕ сообщения. Работает только если есть business_connection_id."""
    ...
    if not conn_id:
        return  # Обычное сообщение, не через Chat Automation

    logger.info(f"Сообщение через Chat Automation от {message.from_user.id}")

    owner_id, owner_chat = await _get_owner_data(context, conn_id)
    if not owner_id or not owner_chat:
        logger.warning(f"Владелец не найден для conn_id={conn_id}")
        return

    sender = message.from_user

    # Не реагируем на себя и владельца
    if sender.id == owner_id:
        return
    if sender.is_bot:
        return

    # ─── 1. АВТООТВЕТ (только на новые, не на отредактированные) ───
    if update.message:
        try:
            await message.reply_text(AUTO_REPLY_TEXT)
            logger.info("Автоответ отправлен")
        except TelegramError as e:
            logger.error(f"Ошибка автоответа: {e}")

    # ─── 2. ПЕРЕСЫЛКА ВЛАДЕЛЬЦУ В ЛС ───
    try:
        ...
        logger.info("Переслано владельцу")
    except TelegramError as e:
...
def main() -> None:
    ...
    # КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: ловим MESSAGE и EDITED_MESSAGE, а не BUSINESS_MESSAGE
    app.add_handler(
        MessageHandler(
